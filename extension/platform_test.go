...
func TestPluginIconNotExists(t *testing.T) {
	...
	assert.Equal(t, "The plugin icon Resources/config/plugin.png does not exist", ctx.errors[0].Message)
}
...
func TestPluginIconIsTooBig(t *testing.T) {
	...
	assert.Equal(t, "The plugin icon Resources/config/plugin.png is bigger than 10kb", ctx.errors[0].Message)
}
